...
bigInt.prime = function (bitLength) {
  ...
  do {
    ...
    console.assert(
      rnd.bitLength() == bitLength,
      "ERROR: " + rnd.bitLength() + " != " + bitLength
    );
  } while (!rnd.isPrime());
  return bigInt(rnd);
};
...
const PaillierPublicKey = class PaillierPublicKey {
  ...
  encrypt(m, r) {
    ...
    //Added r for verify vote
    return [
      this.g
        .modPow(bigInt(m), this._n2)
  ...
  addition(...ciphertexts) {
    return ciphertexts.reduce(
      (sum, next) => sum.multiply(bigInt(next)).mod(this._n2),
      bigInt(1)
    );
  }
  multiply(c, k) {
...
const PaillierPrivateKey = class PaillierPrivateKey {
  ...
  decrypt(c) {
    return L(
      bigInt(c).modPow(this.lambda, this.publicKey._n2),
      this.publicKey.n
    )
      .multiply(this.mu)
